pipeline {
            ...
            steps {
                withCredentials([string(credentialsId: 'vault-token', variable: 'VAULT_TOKEN')]) {
                    script {
                        ...
                        def required = ['DOCKER_USER','DOCKER_PASS','GH_TOKEN','SONAR_TOKEN','DB_PASSWORD','JWT_SECRET','INTERNAL_API_KEY']
        ...
        stage('SonarQube Analysis') {
            steps {
                ...
                withSonarQubeEnv('SonarQube') {
                    sh '''
                        npx sonar-scanner \
                          -Dsonar.projectKey=task-service \
                          -Dsonar.sources=. \
        ...
        stage('Update Manifests') {
            steps {
                sh '''
                    ...
                    sed -i "s|image: houdanasr/task-service:.*|image: houdanasr/task-service:${BUILD_NUMBER}|g" kubernetes/deployment.yaml

                    git add kubernetes/deployment.yaml
                    if ! git commit -m "ci: update image tag to build #${BUILD_NUMBER} [skip ci]"; then
                        echo "⚠️ No changes to commit"
                    fi
                    git push "https://${GH_USER}:${GH_TOKEN}@github.com/NASRHOUDA/task-service.git" HEAD:main
    ...
    post {
        success { echo '✅ Pipeline task-service réussi !' }
